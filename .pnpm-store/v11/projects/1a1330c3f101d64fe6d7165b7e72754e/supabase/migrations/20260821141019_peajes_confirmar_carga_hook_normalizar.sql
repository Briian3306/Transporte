-- F14-2 gap-fill §7.1: peajes_confirmar_carga dispara peajes_normalizar_tarifas
-- por documento confirmado. Firma pública intacta (mismos args / jsonb).
-- Un fallo del clasificador se registra y NO revierte una carga ya validada (RN-12/17).
-- RLS: nota explícita de diferimiento por rol/empresa (PRD §5.2).

COMMENT ON POLICY tarifas_normalizadas_authenticated_all ON public.tarifas_normalizadas IS
  'PRD §5.2 · RLS plana TO authenticated USING true. RLS por rol/empresa diferida.';
COMMENT ON POLICY tarifas_parametros_peaje_authenticated_all ON public.tarifas_parametros_peaje IS
  'PRD §5.2 · RLS plana TO authenticated USING true. RLS por rol/empresa diferida.';
COMMENT ON POLICY tarifas_status_catalogo_authenticated_all ON public.tarifas_status_catalogo IS
  'PRD §5.2 · RLS plana TO authenticated USING true. RLS por rol/empresa diferida.';
...
BEGIN
  ...
  -- F14-2 · hook interno. NC → (0,0). No cambia la firma pública.
  -- auto_confirmar_horario permanece false: el worker rápido no auto-confirma PICO/NO_PICO.
  BEGIN
...
COMMENT ON FUNCTION public.peajes_confirmar_carga(jsonb, jsonb, uuid, jsonb, jsonb, jsonb, text, numeric) IS
  'Confirma carga en documentos (FC|NC); persiste categoria cruda (F14/RN-15); valida Σ neto vs subtotal+bonificacion. Hook F14-2: llama peajes_normalizar_tarifas(documento_id). Firma pública intacta. Fallo del clasificador no revierte la carga. RLS por rol/empresa diferida (PRD §5.2).';
