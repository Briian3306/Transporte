...
`BORRAR_ESPACIOS`, `ELIMINAR_GUIONES`, `CONVERTIR_MAYUSCULAS`, `COMBINAR_COLUMNAS`, `FORMATEAR_FECHA_HORA`, `CALCULAR_IMPORTE_NETO`, `CONVERTIR_NUMERO`, `CONVERTIR_NUMERO_ARS`, `CONVERTIR_TEXTO`, `ASIGNAR_VALOR`, `COPIAR_COLUMNA`, `ELIMINAR_IVA`, `OPERAR_NUMERO`, `REEMPLAZAR_TEXTO`, `FILTRAR_COLUMNA`
...
Migración `20260811190002_peajes_algoritmo_filtrar_columna.sql`: agrega `FILTRAR_COLUMNA` (filtro de filas; p. ej. MERCOSUR `ESTACION=1`).
