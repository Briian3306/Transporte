-- Facturas: cuenta deja de ser obligatoria (wizard Paso 7 / UI).
ALTER TABLE public.facturas
  ALTER COLUMN cuenta DROP NOT NULL;

COMMENT ON COLUMN public.facturas.cuenta IS
  'Cuenta asociada a la factura (opcional). Vacío/NULL permitido.';

-- peajes_confirmar_carga: cuenta opcional (NULLIF vacío → NULL). Cuerpo base = 20260803190348.
...
BEGIN
  ...
  ELSE
    ...
    SET
      ...
      cuenta = CASE
        WHEN p_factura ? 'cuenta' THEN v_cuenta
        ELSE cuenta
      END,
...
COMMENT ON FUNCTION public.peajes_confirmar_carga(jsonb, jsonb, uuid, jsonb, jsonb, jsonb, text, numeric) IS
  'Confirma carga: factura (cuenta opcional) + pasadas + registro. Popula user_id y file_upload_name.';
