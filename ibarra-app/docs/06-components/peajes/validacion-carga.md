...
La regla que habilita avanzar es el contraste entre el **subtotal declarado** de la factura y la suma de `IMPORTE_NETO` de las pasadas: la diferencia absoluta debe ser menor o igual a **$5,00**.
...
validarCarga + detectarDuplicados
        ↓
Checklist diagnóstico → habilitar Paso 9 si no hay bloqueos y |diferencia| ≤ $5
```
...
|---|---|---|---|
| Importe de factura | `abs(subtotal - suma_neta) <= 5` | Sí | 7 |
...
| Diferencia de subtotal mayor a $5 | Total de las pasadas no concilia con el subtotal | Corregir subtotal o importes/mapeo | Bloquea |
...
|---|---|
| `peajes_validar_factura_pasadas` | Suma el arreglo de netos y devuelve `valido`, `diferencia`, `tolerancia`, `suma_pasadas` y `dentro_tolerancia` |
...
Las migraciones `20260804183929_peajes_permitir_neto_declarado_si_total_valido.sql` y `20260804184205_peajes_permitir_neto_declarado_en_pasadas.sql` alinean la persistencia con el Paso 8: `pasadas.importe_neto` conserva el importe declarado (no negativo) y el criterio vinculante es la conciliación del subtotal total.
...
La diferencia está dentro de los $5,00 permitidos, por lo que no debe bloquear el avance por el control de importe. Los netos de fila que reflejan descuentos del proveedor se conservan para alcanzar dicha conciliación.
...
El test SQL `peajes_f01_test.sql` pasó al aplicar las migraciones. La suite completa puede conservar un fallo ajeno de seed AUSOL: estaciones `REVIEW` 19 frente a 18 esperadas.
