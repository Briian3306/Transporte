...
- [Qué resuelve (dos capas)](#qué-resuelve-dos-capas)
...
**Patrón A** — el archivo no trae categoría (`categoria` null): la familia es por estación.  
**Patrón B** — hay categoría del proveedor mapeada en el wizard: la familia es estación + categoría.
...
7. Asignar status → se guarda y se propaga a pasadas.tarifa_status
8. (Opcional) Comparar con grupos similares y aplicar en lote
9. Cuando no queden pendientes, la concesión queda «completa»
```
...
2. `app-data-table` con plantillas `appDataTableColumn="clave"` (no `key="clave"`):
   - Importe
   - Casos
   ...
   - Diagnóstico
   - **Select de status** (`app-tarifa-status-buttons` → `app-search-select`). Única columna editable.
   - **Acciones** — botón **Ver casos** (`fa-search`) abre el diálogo de pasadas del nivel
3. Contador de pendientes y barra de acciones.
...
| **Recalcular** | Ejecuta el recálculo; conserva clasificaciones `confirmado_manual` |
...
`app-dialog` con eyebrow «Auditoría de tarifas». Descripción: vuelve a detectar familias desde las pasadas actuales; **no pisa** confirmaciones manuales. Feedback en barra de aviso mientras corre (puede tardar).
...
`TarifaAsignacion`: `{ tarifa_normalizada_id, status_codigo }`.
...
> Última actualización: 2026-08-13 (select de status + franja `HH:mm – HH:mm` + Ver casos)
